...
func maxProfit(prices []int) int {
	...
	return rec(prices, 1, prices[0], cache)
}

func rec(prices []int, i int, buyPrice int, cache map[int]int) int {
	if i >= len(prices) {
		return 0
	}
	...
	todayPrice := prices[i]
	todayProfit := max(0, todayPrice-buyPrice)

	sellToday := todayProfit + rec(prices, i+2, aLot, cache)
	sellTomorrow := rec(prices, i+1, min(buyPrice, todayPrice), cache)

	res := max(sellToday, sellTomorrow)
